...
namespace my {
    ...
    LogEvent::LogEvent(std::shared_ptr<Logger> logger, LogLevel::Level level, const char *file, int32_t line,
            ...
            : m_logger(logger), m_level(level), m_file(file), m_line(line), m_threadId(threadId), m_fiberId(fiberId),
              m_elapse(elapse), m_time(timestamp), m_threadName(threadName) {
        ...
        for (size_t i = 0; i < m_pattern.size(); i++) {
            ...
            if (m_pattern[i] != 'd') {
                ...
                if (index == std::string::npos) {
                    m_items.push_back(StringFormatItem::ptr(
                            new StringFormatItem("<error format %d{" + m_pattern.substr(i) + " >")));
                    m_error = true;
                    continue;
                }
        ...
        LogIniter() {
            g_log_defines->addListener([](const std::set<LogDefine> &old_val, const std::set<LogDefine> &new_val) {
                ...
                for (auto &i: old_val) {
                    ...
                    if (it == new_val.end()) {
                        auto logger = MY_LOG_NAME(it->name);
                        logger->clearAppender();
