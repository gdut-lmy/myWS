...
//日志事件
    ...
    public:
        ...
        LogEvent(std::shared_ptr<Logger> logger, LogLevel::Level level, const char *file, int32_t line,
                 int32_t threadId, int32_t fiberId, int32_t elapse, int64_t timestamp, const std::string &threadName);
...
//日志格式化
    ...
    public:
        ...
        explicit LogFormatter(
                const std::string &pattern = "%d{%Y-%m-%d %H:%M:%S}%T[ %p%T]%T[%c]%T%t%T%N%T%F%T%T%f:%l%T%m%n");
