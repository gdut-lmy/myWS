...
public:
    ...
    std::string toString() override {
        try {
           ...
           // MY_LOG_ERROR(MY_LOG_ROOT())<<"ConfigVar::toString() exception "<<e.what()<<" convert: "<< typeid(T).name()<<" toString";
        }
        return "";
    }
    ...
    static typename ConfigVar<T>::ptr Lookup(const std::string& name,const T& value,const std::string& description){
        ...
        if(it != GetDatas().end()){
            ...
            if(tmp){
             ...
             //   MY_LOG_ERROR(MY_LOG_ROOT())<<"lookup name="<<name<<" already exist but type not "<< typeid(T).name() <<" real type="<<it->second->getTypeName()<<" value: "<<it->second->toString();
                return nullptr;
            }
        }
