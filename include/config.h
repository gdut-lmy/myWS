...
#ifndef MYWS_CONFIG_H
#define MYWS_CONFIG_H
...
namespace my {
    ...
    public:
        ...
        ConfigVarBase(std::string name, std::string description) : m_name(std::move(name)),
...
//ToStr  std::string operator()(const T&)
    template<class T, class FromStr = LaxicalCast<std::string, T>, class ToStr = LaxicalCast<T, std::string>>
    ...
    public:
        ...
        std::string toString() override {
            try {
                ...
                MY_LOG_ERROR(MY_LOG_ROOT()) << "ConfigVar::toString() exception " << e.what() << " convert: "
                                            << typeid(T).name() << " toString";
        ...
        bool fromString(const std::string &str) override {
            try {
                ...
                MY_LOG_ERROR(MY_LOG_ROOT())<<"ConfigVar::fromString() exception "<<e.what()<<" convert: String to "<< typeid(T).name();
            }
            return false;
        }
    ...
    public:
        ...
        Lookup(const std::string &name, const T &value, const std::string &description) {
            ...
            if (it != GetDatas().end()) {
                ...
                if (tmp) {
                    MY_LOG_INFO(MY_LOG_ROOT()) << "lookup name=" << name << " already exist";
                    return tmp;
                } else {
                    MY_LOG_ERROR(MY_LOG_ROOT()) << "lookup name=" << name << " already exist but type not "
            ...
            if (name.find_first_not_of("abcdefghijklmnopqrstuvwxyz0123456789._")
                != std::string::npos) {
                MY_LOG_ERROR(MY_LOG_ROOT()) << "lookup invalid name " << name;
